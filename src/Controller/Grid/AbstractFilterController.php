...
abstract class AbstractFilterController extends AbstractPhoenixController
{
	...
	{
		$model['list.search'] = $this->getUserStateFromInput($this->getContext('list.search'), 'search', [], InputFilter::ARRAY_TYPE);
		$model['list.filter'] = $this->getUserStateFromInput($this->getContext('list.filter'), 'filter', [], InputFilter::ARRAY_TYPE);
