...
JS;
	...
	public static function babel(callable $condition = null)
	{
		if (!static::inited(__METHOD__))
		{
			...
			if ($condition(WhichBrowserFactory::getInstance()))
			{
				static::addJS(static::phoenixName() . '/js/polyfill/babel-polyfill.min.js');
				static::addJS(static::phoenixName() . '/js/polyfill/babel.min.js');
				static::internalJS(
					<<<JS
Babel.disableScriptTags();
window.addEventListener('DOMContentLoaded', function () {
    var scripts = document.querySelectorAll('script[data-babel]');

    [].forEach.call(scripts, function (e) {
        e.setAttribute('type', 'text/babel');
        e.setAttribute('data-presets', 'es2015,stage-2');
    });

    Babel.transformScriptTags(scripts);
});
JS
				);
			}
